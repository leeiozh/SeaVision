...
#
# Win7-совместимая debug-сборка: то же, что seavision-win7.spec, но С matplotlib
# (Agg-backend, savefig в PNG, без GUI) для режима pics=<путь>.
# Собирается на Python 3.8 + PyInstaller 5.x (см. комментарий в seavision-win7.spec).
...
coll = COLLECT(
    ...
    strip=False,
    upx=False,
    name='seavision',        # → dist-win7-debug/seavision/ (workflow задаёт --distpath)
)
