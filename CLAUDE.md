...
**Состояние алгоритма (`algo_state`, байт 42) и статус-пакеты:** `n_sys` удалён из протокола (число систем восстановимо из `Hs>0`). Manager шлёт статус-пакеты (`ProcessResult.is_status=True`) в фазах ожидания/накопления/сброса/ошибки через `_emit_status()`. **Чтобы не гасить картинку приёмника, heartbeat пересылает ПОСЛЕДНИЙ удачный результат** (`Manager._last_output`, shallow-copy: спектры/Wave разделяются, переопределяются только `algo_state`/`progress`). Нули шлются только до первого результата (фабрика `Output.status()`). CSV-синк статус-пакеты пропускает; UDP-синк шлёт. Реальный результат идёт с `algo_state=2, progress=100` (дефолты `Output`).
...
### Физические ограничения на передаваемые параметры (`src/io/output.py`)

`_phys_clip(o)` вызывается в начале `UdpOutputSink.send()` и `CSVOutputSink.send()` — до любой сериализации. Идемпотентна. При обрезке логирует WARNING.

| Константа | Значение | Обоснование |
|---|---|---|
| `_PHYS_SWH_MAX` | **20.0 м** | Выше максимального надёжно измеренного Hs (~18 м при ураганах) |
| `_PHYS_PERIOD_MAX` | **30.0 с** | Граница гравитационных волн; bin-1 артефакт может давать ~600 с |
| `_PHYS_WSPD_MAX` | **30.0 м/с** | Bf 12 = 32.7 м/с; радарная оценка насыщается раньше |
| `_PHYS_CURR_MAX` | **3.0 м/с** | Совпадает с процессорным клипом `_MAX_CURRENT=3.0 м/с` |

NaN/inf во всех полях → 0.0 (признак «нет данных»). Период 0 (нет волны) не клипается снизу.
...
**Averager — политика усреднения:** усредняются `swh, t_m, d_m` (wave_sum), `wspd, wind_dir, curr_speed, curr_dir`, `spec_1d/spec_2d`. Только с последнего кадра: `t_p, d_p` (wave_sum), все поля `wave_win/sw1/sw2`, `n_sys`. Направления `d_m, wind_dir, curr_dir` — через циркулярное среднее (arctan2), не арифметическое.
