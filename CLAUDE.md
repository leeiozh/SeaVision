...
- `debug_freq_spec.png` — 1D MTF-спектр из `spec_3d_ship` (ship-corrected); цветные вертикальные линии и полупрозрачные полосы для каждого пика из `systems_draft`; аннотации T=X.Xs
- `debug_portrait.png` — Doppler-скорректированный ω-k портрет + кривая `ω=√(gk)` + per-system curves (`u_curr_proj` = проекция истинного тока на направление системы); scatter-точки из `sys_scatter` — цвет по номеру системы, размер ∝ log(интенсивность)
- `debug_spec2d.png` — направленный спектр `s_om_th` + маркеры финальных систем из `calc_partitions` (sum, w_s, sw_1, sw_2 — горизонтальные/вертикальные линии) + кресты `+` для `systems_draft` с частотными полосами
