...
Возвращает `(sig, wdir_deg)`. `wdir` передаётся в `calc_partitions()` только как ориентир классификации (система в пределах ±45° от wdir = ветровая, `WIND_DIR_THRESH=45.0`); `wave_win.d_p` — спектральный пик ветровой системы, а не `wdir` напрямую.
...
| `view_res.py` | Скрипт просмотра результатов батча из `batch_out/params.csv`: scatter-plot `bdir` vs `d_p` и `mdts` vs `d_p`. Черновой инструмент, не синхронизируется. |
